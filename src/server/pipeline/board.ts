...
export function createBoardService(prisma: PrismaClient) {
  ...
  async function transition(
    ...
    return writeTransaction((tx) => transitionIn(tx, projectId, input, caller));
  }
  ...
  async function gate(
    ...
    const written = await writeTransaction(async (tx) => {
      const row = await latestRow(tx, projectId, input.key);
      if (!row) return fail(`no such board item: ${input.key}`);
    ...
    return writeTransaction(async (tx) => {
      ...
      const verifierPassedAfterPlan =
        ...
        (await tx.agentRunStep.findFirst({
          where: { OR: [{ accepted: true }, { accepted: null }], stepId: "verify", outcome: "ok", ...(lastPlan ? { at: { gt: lastPlan.at } } : {}), run: { projectId, agent: PLAN_VERIFIER, key: input.key } },
          select: { id: true },
        })) !== null;
  ...
  async function submitReport(projectId: string, input: { key: string; actor: string; path: string; commit: string }, actorRef: string) {
    return writeTransaction(async (tx) => {
      const row = await latestRow(tx, projectId, input.key);
      if (!row) return fail(`no such board item: ${input.key}`);
  ...
  async function advanceRun(tx: Db, projectId: string, key: string) {
  ...
  async function writeTransaction<T>(work: (tx: Prisma.TransactionClient) => Promise<ServerResult<T>>, options?: { isolationLevel: "Serializable" }): Promise<ServerResult<T>> {
    try {
      return await prisma.$transaction(async (tx) => {
        const result = await work(tx);
        if (!result.ok) throw new BoardWriteFailure(result.reason);
        return result;
      }, options);
    } catch (error) {
      if (error instanceof BoardWriteFailure) return fail(error.message);
      throw error;
    }
  }
  return { latestBoard, walkingKeys, latestBoardWithEvents, latestRowFor, availableBacklogCount, backlogWithStatus, getWithHistory, hasHistoryBefore, propose, transitionIn, transition, discard, gate, recordValidation, submitPlan, submitReport, advanceRun, resetRun, advancePipeline };
}
class BoardWriteFailure extends Error {}
...
async function claim(tx: Db, row: { id: string; status: string; updatedAt: Date }, data: Prisma.BoardItemUpdateManyMutationInput): Promise<void> {
  ...
  if (result.count !== 1) throw new BoardWriteFailure("stale");
}
export const { latestBoard, walkingKeys, latestBoardWithEvents, latestRowFor, availableBacklogCount, backlogWithStatus, getWithHistory, hasHistoryBefore, propose, transitionIn, transition, discard, gate, recordValidation, submitPlan, submitReport, advanceRun, resetRun, advancePipeline } = createBoardService(defaultDb);
