...
export async function addBacklogItem(slug: string, _prev: BacklogFormState, form: FormData): Promise<BacklogFormState> {
  ...
  if (!BACKLOG_KEY_RE.test(key)) return { status: "error", error: "Key must look like FEAT-01: capital letters, a dash, a number." };
...
export async function updateBacklogItem(
  ...
  if (updated.count === 0) return { status: "error", error: `${key} doesn't exist.` };
  revalidatePath(projectPath(slug, "/backlog"));
  return { status: "saved" };
}
...
export async function removeBacklogItem(slug: string, key: string): Promise<BacklogFormState> {
  ...
  if (open.some((row) => row.backlogItem.key === key)) {
    return { status: "error", error: `${key} is open on the board. Finish or discard it before removing.` };
  }
  const removed = await prisma.backlogItem.updateMany({
    where: { projectId, key, removedAt: null },
    data: { removedAt: new Date() },
  });
  if (removed.count === 0) return { status: "error", error: `${key} doesn't exist.` };
  revalidatePath(projectPath(slug, "/backlog"));
  return { status: "saved" };
}
