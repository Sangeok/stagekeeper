...
프로젝트의 데이터와 사용 여부를 관리하는 유일한 개인 사용자. 개인 플랜에는 초대·공유·Member 역할이 없다.
_Avoid_: Member, 플랜 Owner, 저장소 소유자

**저장소 소유자 (Repository Owner)**:
연결된 GitHub 저장소가 속한 개인 또는 조직 계정. 프로젝트 소유자와 다른 주체일 수 있다.
_Avoid_: 프로젝트 소유자

**사용 가능 (Available)**:
소유자가 현재 작업에 사용할 수 있는 프로젝트 목록에 포함된 상태.
_Avoid_: Active, Locked

**선택되지 않음 (Not Selected)**:
데이터와 연결을 보유하지만 현재 작업할 프로젝트 목록에는 포함되지 않은 상태. 다시 선택하면 기존 작업을 이어갈 수 있다.
_Avoid_: Paused, Locked, 삭제됨
